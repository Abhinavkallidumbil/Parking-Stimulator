...
public class WayPointEditor 
{
    ...
    public static void OnDrawSceneGizmos(WayPoint waypoint,GizmoType gizmoType)
    {
        ...
        //line from previous to nexr

        if (waypoint.prevoiusWaypoint != null)
        {
            Gizmos.color = Color.red;
            Vector3 offset = waypoint.transform.right * waypoint.waypointWidth / 2f;
            Vector3 offsetTo = waypoint.prevoiusWaypoint.transform.right * waypoint.prevoiusWaypoint.waypointWidth / 2f;

            Gizmos.DrawLine(waypoint.transform.position + offset , waypoint.prevoiusWaypoint.transform.position + offsetTo);
        }
